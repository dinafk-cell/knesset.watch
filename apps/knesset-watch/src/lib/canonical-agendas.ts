 ...
 *
 * נכתב כמודול נפרד ולא כעריכה של agendas.ts, כי agendas.ts שירת אז את
 * /agendas ואת /api/agenda-votes ולא היה אפשר לשבור אותם. מאז /agendas
 * נבנה מחדש על הטקסונומיה הזאת, ו-agendas.ts, /api/agenda-votes ושני
 * רכיבי הלקוח הישנים הוסרו (10.9.2026). זה המודול היחיד שנשאר.
 *
 ...
 */
/**
 * הסף שמתחתיו ציר אינו מוצג בשאלון כלל.
 *
 * מיוצא כדי שסקריפטים שמייצרים צירים חדשים יסננו לפיו. בלי זה נוצרו
 * עשרה צירים עם ארבע הצעות כל אחד: הם נכתבו לקטלוג ולמסד, ההצעות
 * נראו מסווגות, ואף משתמשת לא יכלה להגיע אליהם.
 */
export const MIN_BILLS_PER_ISSUE = 5;
