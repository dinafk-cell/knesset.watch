  ...
  return (
    <div className="min-h-screen bg-paper" dir="rtl">
      <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <h3 className={`text-body font-medium leading-snug ${selected ? 'text-accent-ink' : ''}`}>
                        {d.label}
                      </h3>
                      {selected && <span className="text-ui font-medium text-accent">✓</span>}
                    </div>
                    <p className={`text-meta mt-1 font-medium ${selected ? 'text-accent' : 'text-mute'}`}>
        ...
        {step === 'issues' && (
          <div>
            <h2 className="text-section font-medium mb-1">מה העמדה שלך בכל נושא?</h2>
            <p className="text-ui text-mute mb-5 font-medium">
              ...
              {issuesByDomain.map(({ domain, issues }) => (
                <section key={domain.id}>
                  ...
                  <div className="flex items-center gap-2 mb-3 pb-2 border-b-2 border-accent">
                    ...
                    <h3 className="text-body font-medium text-accent-ink">{domain.label}</h3>
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-card border-r-4 border border-line p-4 transition-colors ${
                            chosen ? 'border-r-accent bg-accent-wash' : 'border-r-line'
                          }`}
                        >
                          <div className="flex items-baseline gap-2">
                            ...
                            <h4 className="text-body font-medium">{issue.label}</h4>
                          </div>
                ...
                */}
                ...
                <ol className="flex flex-col gap-3">
                  {rows.map((row, idx) => {
                    ...
                    return (
                      <li key={row.mkId} className="rounded-card border border-line overflow-hidden">
                        <button
                                ...
                                >
                                  <div className="flex items-baseline justify-between gap-2">
                                    <div className="flex items-baseline gap-2">
                                      ...
                                      <h4 className="text-ui font-medium">{a.label}</h4>
                                    </div>
                                    <span className="text-ui font-medium tabular-nums">{a.score}</span>
                                  </div>
