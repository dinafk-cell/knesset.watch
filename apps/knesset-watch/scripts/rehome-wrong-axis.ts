 ...
 * ── הכלל המרכזי ──────────────────────────────────────────────────────
 *
 * "ציר שגוי" אינו "צריך למחוק". הביקורת רואה ציר אחד בכל פעם ואינה
 * אומרת לאן ההצעה כן שייכת. מחיקה בלי חלופה הופכת הצעה מסווגת-לא-טוב
 * להצעה יתומה, וזו החמרה ולא תיקון.
 *
 * לכן כאן: לכל הצעה שנפסלה מחפשים ציר חלופי מתוך 182 הקיימים, ורק
 * אם נמצא ציר בביטחון גבוה ההחלפה תישקל. אחרת ההצעה נשארת במקומה
 * הנוכחי, על כל פגמיו.
 *
...
function buildPrompt(bills: Array<{ id: number; title: string; change: string; oldQ: string }>): string {
  ...
  const billList = bills.map(b =>
    `[${b.id}] ${b.title}\n     ${b.change.slice(0, 240)}\n     (שויכה עד כה ל: ${b.oldQ} — שיוך שנמצא שגוי)`).join('\n\n');
  return `להלן צירי המחלוקת הקיימים באתר:

${axisList}

והלן הצעות חוק שהשיוך הנוכחי שלהן נמצא שגוי:
...
כללים:
- אל תבחר בציר שההצעה כבר משויכת אליו; הוא נבדק ונפסל.
...
async function main(): Promise<void> {
  ...
  console.log(`  ללא בית      : ${all.length - found.length}  ← נשארות במקומן הנוכחי`);
  ...
  for (const r of all) {
    ...
    lines.push([r.billId, t, AX_Q.get(r.oldIssueId),
      r.newIssueId ? AX_Q.get(r.newIssueId) : '(לא נמצא — נשארת במקומה)',
